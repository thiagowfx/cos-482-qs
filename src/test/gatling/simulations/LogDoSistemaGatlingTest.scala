...
/**
 * Performance test for the LogDoSistema entity.
 */
class LogDoSistemaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the LogDoSistema entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all logDoSistemas")
            .get("/api/log-do-sistemas")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created logDoSistema")
                .get("${new_logDoSistema_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created logDoSistema")
            .delete("${new_logDoSistema_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
